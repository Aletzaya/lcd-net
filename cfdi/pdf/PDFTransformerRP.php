...
class PDFTransformerRP {
    ...
    public static function getPDF($Comprobante, $doc, $type, $logo = 'img/logo.png', $path = './', $addrEmisor = NULL, $addrReceptor = NULL) {
        ...
        $pagos = NULL;
        foreach ($Comprobante->getComplemento()->getAny() as $complemento) {
...
class CUSTOM_PDF_RP extends TCPDF {
    ...
    public function Footer() {
        ...
        foreach ($this->Comprobante->getAddenda() as $addenda) {
            ...
            /* @var $addenda Comprobante33\addenda\Observaciones */
        ...
        $pagos = NULL;
        foreach ($this->Comprobante->getComplemento()->getAny() as $complemento) {

            error_log(get_class($complemento));
            if ($complemento instanceof Pagos) {
                $pagos = $complemento; 
            }
        }        
