...
class PDFTransformerCP {

    /**
     * 
     * @param Comprobante33 $Comprobante
     */
    public static function getPDF($Comprobante, $doc, $type, $logo = 'img/logo.png', $path = './', $addrEmisor = NULL, $addrReceptor = NULL) {

        if (!defined("PDF_PAGE_FORMAT")) define("PDF_PAGE_FORMAT", "A4");
        $pdf = new CUSTOM_PDF_CP(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true);

        $pdf->setAddrReceptor($addrReceptor);
        $pdf->setAddrEmisor($addrEmisor);
        ...
        foreach ($Comprobante->getConceptos()->getConcepto() as $Concepto) {
            $html .= "<tr style=\"font-size: 8; text-align: center;\">"
                    ...
                    . "<td style=\"width: 2cm;\">" . (fmod($Concepto->getCantidad(), 1) === 0.00 ? floor($Concepto->getCantidad()) : $Concepto->getCantidad()) . " " . $Concepto->getClaveUnidad() . "</td>"
        ...
        foreach ($Comprobante->getComplemento() as $complemento) {
            ...
            if ($complemento instanceof CartaPorte20) {
                error_log("Found carta porte");
                $htmlp = (new ProcessCartaPortePDF($complemento))->process();
            }
        }
...
class CUSTOM_PDF_CP extends TCPDF {
    ...
    function setAddrEmisor($addrEmisor) {
        $this->addrEmisor = $addrEmisor;
    }

    function setAddrReceptor($addrReceptor) {
        $this->addrReceptor = $addrReceptor;
    }
    ...
    private function decodeUsoDelCFDI($UsoDelCFDI) {
        switch ($UsoDelCFDI) {
            ...
            case "CP01": return "Pagos";
            case "CP02": return "Nómina";
        }
    }
    ...
    public function Header() {
        ...
        $html = "<table style=\"width: 100%; border-collapse: separate; border-spacing: 10px;\">"
                . "<tr>"
                . "<td>"
                ...
                . (empty($this->addrEmisor) ? "" : "<br/><font size=\"8\"><b>" . $this->addrEmisor . "</b></font>")
                . "</td>"
                ...
                . ($this->Comprobante->getVersion() === "4.0" ?
                ("<tr><td><font size=\"8\"><b>Régimen Fiscal " . $this->Comprobante->getReceptor()->getRegimenFiscalReceptor() . "</b> - " . $this->decodeRegimenFiscal($this->Comprobante->getReceptor()->getRegimenFiscalReceptor()) . "</font></td></tr>") : "")
                . ($this->Comprobante->getVersion() === "4.0" ?
                ("<tr><td><b>Domicilio Fiscal</b> " . $this->Comprobante->getReceptor()->getDomicilioFiscalReceptor() . "</td></tr>") : "")
                ...
                . (empty($this->addrReceptor) ? "" : "<tr><td><b>Dirección</b> - " . $this->addrReceptor . "</td></tr>")
                . "</table>"
                . "</td>"
                . "<td style=\"width: 10cm;\">"
                . "<table>"
    ...
    public function Footer() {
        ...
        $converter = new NumericalCurrencyConverter(new SpanishNumbers(),
                $this->Comprobante->getMoneda() == 'MXN' ? new Currency('PESOS', 'PESO') : new Currency('DOLARES', 'DOLAR'));
        ...
        if ($hasObservaciones) {
            ...
            //echo $html;
            $this->SetY(-100 - $sizeObservaciones * 3);
            $this->SetFont('helvetica', '', 7);
            $this->writeHTML($html);
        }

        $html = "<table style=\"font-size: 8; font-weight: bold;\">"
                . "<tr>"
        ...
        if ($this->Comprobante->getCfdiRelacionados() != NULL && !empty($tipoRelacion = $this->Comprobante->getCfdiRelacionados())) {

            $html = "<table style=\"border: 0.1px solid #D2D1D2; font-size: 7; font-weight: bold;\">";

            $tipoRelacion = $this->Comprobante->getCfdiRelacionados()[0]->getTipoRelacion();
            /* @var $CfdiRelacionado cfdi33\Comprobante\CfdiRelacionados\CfdiRelacionado */
            foreach ($this->Comprobante->getCfdiRelacionados()[0]->getCfdiRelacionado() as $CfdiRelacionado) {
                $html .= "<tr>"
        ...
        $html = "<table padding-left: 5px;>"
                ...
                . "<tr><td style=\"font-size: 6;\">" . $this->Comprobante->getTFDOriginalBytes() . "</td></tr>"
