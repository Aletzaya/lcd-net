...
require_once ("com/softcoatl/cfdi/utils/CurlFactory.php");
require_once ("com/softcoatl/cfdi/utils/Commons.php");

use com\softcoatl\cfdi\utils\CurlFactory;
use com\softcoatl\cfdi\utils\Utils;
...
class CFDIValidator {
    ...
    static function CallAPI($expresion, $url = 'https://consultaqr.facturaelectronica.sat.gob.mx/ConsultaCFDIService.svc?wsdl') {
        ...
        try {
            ...
            $curl = CurlFactory::create($url, $post, $headers);
            $soap = curl_exec($curl);
            $err = curl_error($curl);
            curl_close($curl);
