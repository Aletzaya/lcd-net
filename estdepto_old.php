...
/*
  ...
  $TmFecha = "and ot.fecha >= '$Fecha'";
  } else {
  $TmFecha = "and ot.fecha >= '" . $_REQUEST[FechaIni];
  if ($_REQUEST[FechaFin] == "") {
  $TmFechaf = "'AND ot.fecha <= '" . $Fecha . "'";
  } else {
  $TmFechaf = "'AND ot.fecha <= '" . $_REQUEST[FechaFin] . "'";
  }
  }
 */
...
/*
  ...
  $cSql="SELECT ot.orden,ot.institucion,cli.nombrec,ot.fecha,ot.hora,est.estudio,est.descripcion,ot.cliente,ot.entemailmed,ot.entemailinst,otd.usrest,otd.statustom,otd.alterno,otd.estudio,otd.capturo,otd.fr,otd.creapdf,otd.usrvalida,est.depto,est.depto,cli.nombrec,otd.proceso
  FROM ot
  INNER JOIN otd on otd.orden=ot.orden
  INNER JOIN est on est.estudio=otd.estudio
  INNER JOIN cli on cli.cliente=ot.cliente
  WHERE ot.cliente=cli.cliente AND otd.orden='$Folio'";
                ...
                <td height="380" valign="top">
                    ...
                    while ($rg = mysql_fetch_array($res)) {
                        ...
                        if ($rg[capturo] <> '' and $rg[fr] == '0') {
                            ...
                            if ($rg[depto] <> 2) {
                                if ($rg[creapdf] == 'pdf') {
                                    echo "<td class='Seleccionar' align='right'><a class='edit' href=javascript:winuni('resultapdf.php?cInk=$clnk&Orden=$rg[orden]&Estudio=$rg[estudio]&Depto=TERMINADA&op=im&alterno=$rg[alterno]')><i class='fa fa-print fa-lg' aria-hidden='true' style='color:#2E86C1'></i></a></td>";
                                } else {
                                    echo "<td class='Seleccionar' align='right'><a class='edit' href=javascript:winuni('resultapdf.php?cInk=$clnk&Orden=$rg[orden]&Estudio=$rg[estudio]&Depto=TERMINADA&op=im&alterno=$rg[alterno]')><i class='fa fa-print fa-lg' aria-hidden='true' style='color:#2E86C1'></i></a></td>";
                                }
                            } else {
