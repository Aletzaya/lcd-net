...
if($Opcion=='Detalle'){
    ...
    $cSql2="SELECT otd.capturo,ot.fecha as fechaot,ot.hora as horaorden,ot.orden,ot.suc,ot.institucion,otd.estudio,est.depto,ot.cliente,cli.nombrec as paciente,inst.alias as alias,est.descripcion
    from ot
    ...
    $cSql2="SELECT otd.capturo,ot.fecha as fechaot,ot.hora as horaorden,ot.orden,ot.suc,ot.institucion,otd.estudio,est.depto,ot.cliente,cli.nombrec as paciente,inst.alias as alias,est.descripcion,count(*) as cant,est.subdepto
    from ot
    INNER JOIN otd on otd.orden=ot.orden
    INNER JOIN est on est.estudio=otd.estudio
    INNER JOIN cli on ot.cliente=cli.cliente
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <table width='99%' align='center' border='0' cellpadding='1' cellspacing='2' bgcolor='#f1f1f1' style='border-collapse: collapse; border: 0px solid #999;'>
      ...
      <tr>
          <td class='letrap'><font size='+2'>Captura: <?= $_REQUEST[usuario] ?></font></td>
      </tr>
      <tr>
          <td class='letrap'>Relacion de ordenes Capturadas del <?= $FechaI ?> al <?= $FechaF ?> </td>
      </tr>
      </table>
    ...
    <table width='99%' align='center' border='0' cellpadding='1' cellspacing='2' style='border-collapse: collapse; border: 0px solid #999;'>
      <tr>
          <td class='letrap' align='right'>
            ...
            if($Opcion=='Detalle'){
            ?>
                <a href='repcaptura.php?FecI=<?=$FechaI?>&FecF=<?=$FechaF?>&usuario=<?=$capt?>&Sucursal=<?=$filtro3?>&Institucion=<?=$filtro5?>&Depto=<?=$filtro7?>&Opcion=Resumen'>
                Resumen<a>
            <?php
            }else{
            ?>
                <a href='repcaptura.php?FecI=<?=$FechaI?>&FecF=<?=$FechaF?>&usuario=<?=$capt?>&Sucursal=<?=$filtro3?>&Institucion=<?=$filtro5?>&Depto=<?=$filtro7?>&Opcion=Detalle'>
                Detalle<a>
            <?php
            }
            ?>
