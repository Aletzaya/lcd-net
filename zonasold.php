...
$op        = $_REQUEST[op];
$Msj       = $_REQUEST[Msj];
$Id        = 8;             //Numero de query dentro de la base de datos
//
 ...
 */
 ...
 $lUp  = mysql_query("DELETE FROM zns WHERE id='$_REQUEST[cId]' limit 1");
 $Msj  = "Registro eliminado";
...
if(strlen($Qry[filtro])>8){$Dsp='Filtro activo';}
...
echo '<td height="380" valign="top">';
    ...
    while($rg=mysql_fetch_array($res)){
        ...
        if($RetSelec <>''){
            echo "<td class='Seleccionar' align='center'><a class='edit' href='$RetSelec?Paciente=$rg[zona]'>Seleccionar</a></td>";
        }else{
            echo "<td class='Seleccionar' align='center'><a class='edit' href='$cLink?busca=$rg[zona]'>Editar</a></td>";
        }
