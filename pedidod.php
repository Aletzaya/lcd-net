...
if ($_REQUEST["op"] === "delete") {
    if (mysql_query("DELETE FROM pedidod WHERE idnvo = $_REQUEST[Id]")) {
        Totaliza($busca);
        header("Location: pedidod.php?busca=$busca&Msj=Exito");
    }
    ...
    $Sql = "INSERT INTO pedidod (id,clave,cantidad,costo,existencia) 
                 VALUES 
                 ('$busca','$_REQUEST[Clave]',$_REQUEST[Cantidad],'$Inv[costo]','$Inv[existencia]')";
    echo $Sql;
    $Up = mysql_query($Sql);
...
#Variables comunes;
$QryA = mysql_query("SELECT campos,froms,edi,tampag,filtro FROM qrys WHERE id=42");
$Qry = mysql_fetch_array($QryA);
$CpoA = mysql_query("SELECT $Qry[campos],pedidod.idnvo FROM pedidod LEFT JOIN invl ON pedidod.clave = invl.clave WHERE pedidod.id = $busca");
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <meta charset="UTF-8">
            ...
            <title>Detalle del pedido no. <?= $busca ?></title>
                    ...
                    <table width='98%' align='center' border='0' cellpadding='1' cellspacing='2' bgcolor='#f1f1f1' style='border-collapse: collapse; border: 1px solid #999;'>  
                        <form name='form1' method='get' action="<?= $_SERVER['PHP_SELF'] ?>" onSubmit='return ValidaCampos();'>
                            <tr style="background-color: #2c8e3c">
                                ...
                                <?php
                                $HeSql = "SELECT pedido.id, pedido.fecha, pedido.hora, pedido.cantidad, pedido.importe, pedido.status,
                        ...
                        while ($rst = mysql_fetch_array($resutl)) {
                            ...
                            <tr bgcolor="#<?= $Fdo ?>" height='25px'>
                                ...
                                <?php if ($He["status"] == "ABIERTA") { ?>
                                    <td align='center'><a href="pedidod.php?busca=<?= $busca ?>&op=delete&Id=<?= $rst[idnvo] ?>" class="edit"><i class="fa fa-trash-o fa-2x" aria-hidden="true"></i></a></td>
                                    <?php
                                } else {
                                    echo "<td></td>";
                                }
                            ...
                            <table align="right" width="40%" id="TablaBusqueda">
                                <tr align="left" class="letrap">
                                    ...
                                    <td>Producto : <input type='text' name='Clave' size='20' value='<?= $_REQUEST[nvoId] ?>' class="letrap"></input>
                                        <a href='invlab4.php?busca=ini&orden=invl.descripcion&miBusca=<?= $busca ?>'><i class="fa fa-search fa-2x" aria-hidden="true"></i></a></td>
