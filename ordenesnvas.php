...
$link = conectarse();
//$_REQUEST["Venta"] == 0 ? header("Location: ordenesnvas.php?Venta=". $_SESSION[cVarVal][0]) : "";
...
if ($_REQUEST[op] == "br") {  // Borramos lo que esta en su nueva cita
    ...
    $InsA = mysql_query("SELECT lista,descuento,condiciones,msjadministrativo,enviomail,status FROM inst WHERE status='Activo' and institucion='$prd[0]'");
    ...
    if ($Ins[lista] <> '') {
        ...
        $Obsr = $Ins[msjadministrativo];
    } else {

        $Msj = "Institucion: La institución no existe, Favor de verificar";
    }
...
//*******************************************************
    ...
    $Hsql2 = "SELECT fechaest FROM otdnvas WHERE otdnvas.usr='$Gusr' AND otdnvas.venta='$Vta' order by fechaest Desc limit 1";
    $HeA2 = mysql_query($Hsql2);
    $He2 = mysql_fetch_array($HeA2);

    if (!mysql_query($Hsql2)) {
        $FechaEnthoy = date("Y-m-d");         //Convierto El resultado en Tipo Fecha
        $Fec_entregah = $FechaEnthoy;
    } else {
        $Fec_entregah = $He2[fechaest];
    }

    $cSql = "UPDATE otnvas SET fechae='$Fec_entregah' WHERE usr='$Gusr' and venta='$Vta'";
    EjecutaSql($cSql, 'otnvas');
...
//*******************************************************
    ...
    $Hsql2 = "SELECT lista,fechae FROM otnvas WHERE otnvas.usr='$Gusr' AND otnvas.venta='$Vta'";
    $HeA2 = mysql_query($Hsql2);
    $He2 = mysql_fetch_array($HeA2);
    ...
    if ($He2[lista] <> 0) {
        ...
        $sql = "SELECT id,estudio,lt1,lt2,lt3,lt4,lt5,lt6,lt7,lt8,lt9,lt10,lt11,
                   lt12,lt13,lt14,lt15,lt15,lt15,lt16,lt17,lt18,lt19,lt20,lt22,lt21,lt23,entord as tiempo,descripcion
        ...
        if ($Lta[id] <> '') {
            ...
            $Dias = $Lta[tiempo] . " days";

            $FechaEnthoy = date("Y-m-d");         //Convierto El resultado en Tipo Fecha
            $cHoyh = strtotime($FechaEnthoy);                //Convierto la fecha a Numeros    

            $nDias = strtotime($Dias, $cHoyh);     //puede ser days month years y hasta -1 month menos un mes...
            $Fechaest = date("Y-m-d", $nDias);

            error_log($Lta[lt20]);
            $cSql = "INSERT INTO otdnvas (usr,venta,estudio,precio,descuento,descripcion,fechaest) VALUES 
                        ('$Gusr','$Vta','$Lta[estudio]','$Precio',$rlt[descuento],'$Lta[descripcion]','$Fechaest')";
            EjecutaSql($cSql, 'otdnvas');
//*******************************************************
            $Horaeh = "15:00";
            $Fechah = $He2[fechae];
            ...
            $cHoyh = strtotime($FechaEnthoy);                //Convierto la fecha a Numeros    
            $Diash = $Lta[tiempo] . " days";
            $FechaEnth = strtotime($Diash, $cHoyh);   //Le quito los dias k quiero,  puede ser days month years y hasta -1 month menos un mes...
            $FechaEnth = date("Y-m-d", $FechaEnth);         //Convierto El resultado en Tipo Fecha

            if ($FechaEnth > $Fec_entregah) {
                $Fec_entregah = $FechaEnth;
            }

            $cSql = "UPDATE otnvas SET fechae='$Fec_entregah',horae='$Horaeh' WHERE usr='$Gusr' and venta='$Vta'";
            EjecutaSql($cSql, 'otnvas');
                        ...
                        <tr>
                            <td class="content2" width="33%" height="30">
                                <table width="100%" border="0" align="center" cellpadding="0" cellspacing="2">
                                    ...
                                    <?php
                                    $Csql = "SELECT est.estudio,est.descripcion,otdnvas.precio,otdnvas.descuento,otdnvas.estudio as idestudio, est.entord as tiempo
                                        FROM otdnvas
