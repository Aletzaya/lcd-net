...
if (isset($_REQUEST["busca"])) {
    ...
    if ($_REQUEST["busca"] == "ini") {
        ...
        $_SESSION["OnToy"] = array('', '', 'sl.id', 'Asc', $Retornar, $Fecha, $Fecha, '');   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
if($Palabras > 1){
 ...
 for ($i = 0; $i < $Palabras; $i++) {
        if(!isset($BusInt)){$BusInt=" sl.recibio like '%$P[$i]%' ";}else{$BusInt=$BusInt." and sl.recibio like '%$P[$i]%' ";}
 }
 //$Suc='*';

}else{
        $BusInt=" sl.recibio like '%$busca%' ";  
// $Suc='*';
}
...
if( $busca == ''){
    ...
    $cSql = "SELECT $Qry[campos] FROM $Qry[froms] WHERE sl.fecha>='$FechaI' and sl.fecha<='$FechaF'"; 

}elseif( $busca < 'a'){

    $cSql = "SELECT $Qry[campos] FROM $Qry[froms] WHERE  sl.id='$busca'";

}else{

    $cSql = "SELECT $Qry[campos] FROM sl  WHERE $BusInt $Qry[filtro]";
}
    ...
    <?php
        ...
        while ($rg = mysql_fetch_array($res)) {
            ...
            if ($rg["status"] == "CERRADA") {
                echo "<td class='letrap' align='center'><a class='edit' href=javascript:winuni('impsallabpdf.php?busca=$rg[id]') ><i class='fa fa-print fa-lg' aria-hidden='true'></i></a></td>";
            } else {
                echo "<td class='letrap' align='center'><a class='edit' href='#' ><i class='fa fa-print fa-lg' aria-hidden='true' style='color:red'></i></a></td>";
            }
