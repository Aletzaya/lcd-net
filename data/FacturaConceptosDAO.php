...
class FacturaConceptosDAO {
    ...
    public function retrieveConceptos($folio) {
        ...
        $sql = "SELECT 
                        ...
                        CAST(SUBQ.iva AS DECIMAL(10, 6)) factoriva,
                        CAST(SUBQ.ieps AS DECIMAL(10, 6)) factorieps,
                        round(SUBQ.total, 2) - round(SUBQ.ivadescuento, 2) - round(SUBQ.impiva, 2) - round(SUBQ.impieps, 2) subtotal,
                        round(SUBQ.impiva, 2) impiva,
                        round(SUBQ.impieps, 2) impieps,
                        round(SUBQ.impdescuento, 2) descuento,
                        round(SUBQ.total, 2) - round(SUBQ.impdescuento, 2) - round(SUBQ.ivadescuento, 2) total
                FROM(
                                SELECT 
                                        fcd.idnvo,
                                        fcd.producto,
                                        ...
                                        fcd.cantidad,
                                        fcd.descuento,
                                        ( fcd.cantidad * fcd.preciob ) total,
                                        ( fcd.cantidad * fcd.precio ) subtotal,
                                        ( fcd.cantidad * fcd.precio * (1 - descuento/100) ) base_iva,
                                        ( fcd.cantidad * fcd.precio ) base_ieps,
                                        ( fcd.cantidad * fcd.precio * (1 - descuento/100) * fcd.iva ) impiva,
                                        ( fcd.cantidad * fcd.ieps ) impieps,
                                        ( fcd.cantidad * fcd.precio * fcd.iva * descuento/100) ivadescuento,
                                        ( fcd.cantidad * ( fcd.precio + fcd.ieps ) * descuento/100) impdescuento
                                FROM fcd JOIN inv ON fcd.producto=inv.id
                                WHERE fcd.id = ".$folio.") SUBQ
                LEFT JOIN com ON com.descripcion LIKE SUBQ.descripcion";
         */
        /*
        $sql = "
            SELECT
                ...
                ticket,
                SUBQ.iva,
                CAST(factoriva AS DECIMAL(10, 6)) factoriva,
                SUBQ.ieps,
                CAST(factorieps AS DECIMAL(10, 6)) factorieps,
                umedida,
                inv_cproducto,
                inv_cunidad,
                cantidad,
                total,
                descuento,
                cantidad * ( preciob - SUBQ.ieps ) / (1 + SUBQ.iva) subtotal,
                cantidad * ( preciob - SUBQ.ieps ) / (1 + SUBQ.iva) * SUBQ.iva impiva,
                impieps,
                ( preciob - SUBQ.ieps ) / (1 + SUBQ.iva) precio
            FROM (
                SELECT 
                    fcd.producto,
                    fcd.preciob,
                    ...
                    fcd.ticket,
                    fcd.iva,
                    fcd.iva as factoriva,
                    fcd.ieps,
                    fcd.ieps as factorieps,
                    inv.id,
                    inv.umedida,
                    inv.inv_cproducto,
                    inv.inv_cunidad,
                    CASE
                        WHEN fcd.tipoc = 'I' THEN
                            ( fcd.importe / fcd.preciob )
                        ELSE
                            ( fcd.cantidad )
                    END AS cantidad,
                    CASE
                        WHEN fcd.tipoc = 'I' THEN
                            ( fcd.importe )
                        ELSE
                            ( fcd.cantidad * fcd.preciob )
                    END AS total,
                    ( fcd.cantidad * fcd.precio * descuento/100) descuento,
                    CASE
                        WHEN fcd.tipoc = 'I' THEN
                            ( ( fcd.importe / fcd.preciob ) * fcd.ieps )
                        ELSE
                            ( fcd.cantidad * fcd.ieps )
                    END AS impieps,
                    fcd.precio precio
                FROM fcd JOIN inv ON fcd.producto=inv.id
                WHERE fcd.id = " . $folio .
