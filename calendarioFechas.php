...
$Pb = "SELECT fechas.observaciones,fechas.fecha,emp.nombre,fechas.tipo FROM fechas LEFT JOIN emp"
        . " ON emp.id=fechas.empleado ORDER BY fechas.id DESC;";
$cCpo = mysql_query($Pb);
While ($Cpo = mysql_fetch_array($cCpo)) {
    ...
    if ($Cpo["tipo"] === "Cumple") {
        $Color = "#239B56";
    } else if ($Cpo["tipo"] === "Aniversario") {
        $Color = "#D4AC0D";
    } else {
        $Color = "#2874A6";
    }
    $Gen = $Cpo["observaciones"] . " - " . $Cpo["nombre"] . " - " . $Cpo["tipo"];
    $Stg = $Stg . " { title : \" $Gen \", start : \"$Fecha\", end : \"$Cpo[fecha]\",color : \"$Color\"},";
}
