...
if (isset($_REQUEST["busca"])) {
    ...
    if ($_REQUEST["busca"] == "ini") {
        ...
        $_SESSION["OnToy"] = array('', '', 'pedido.id', 'Asc', $Retornar, $Fecha, $Fecha, '');   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
if($Palabras > 1){
 ...
 for ($i = 0; $i < $Palabras; $i++) {
        if(!isset($BusInt)){$BusInt=" pedido.recibio like '%$P[$i]%' ";}else{$BusInt=$BusInt." and pedido.recibio like '%$P[$i]%' ";}
 }
 //$Suc='*';

}else{
        $BusInt=" pedido.recibio like '%$busca%' ";  
// $Suc='*';
}
...
if( $busca == ''){
    ...
    $cSql = "SELECT $Qry[campos] FROM $Qry[froms] WHERE pedido.fecha>='$FechaI' and pedido.fecha<='$FechaF'"; 

}elseif( $busca < 'a'){

    $cSql = "SELECT $Qry[campos] FROM $Qry[froms] WHERE  pedido.id='$busca'";
    ...
    <?php
        ...
        while ($rg = mysql_fetch_array($res)) {
            ...
            if ($rg["status"] == "CERRADA") {
                echo "<td class='letrap' align='center'><a class='edit' href=javascript:winuni('imppedidopdf.php?busca=$rg[id]') ><i class='fa fa-print fa-lg' aria-hidden='true'></i></a></td>";
            } else {
