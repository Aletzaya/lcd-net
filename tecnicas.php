...
if (isset($_REQUEST["busca"])) {
    ...
    if ($_REQUEST["busca"] == "ini") {
        ...
        $_SESSION["OnToy"] = array('', '', 'tec.id', 'Asc', $Retornar);   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
if( $busca == ''){
    ...
    $cSql = "SELECT $Qry[campos] FROM tec WHERE $BusInt $Qry[filtro]";
}
...
if($_REQUEST["Op"] === "Elimina"){
    $Sql ="DELETE FROM tec WHERE id = $_REQUEST[nvoId]";
    if(mysql_query($Sql)){
        header("Location: tecnicas.php?busca=ini&Msj=Registro eliminado con exito !");
    }
}
...
//Tabla contenedor de brighs
    ...
    while ($rg = mysql_fetch_array($res)) {
        ...
        echo "<td class='Seleccionar' align='center'><a class='edit' href='tecnicas.php?nvoId=$rg[id]&Op=Elimina'><i class='fa fa-trash-o fa-2x' aria-hidden='true'></i></a></td>";
