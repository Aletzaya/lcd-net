...
$CapA = mysql_query("select impest,statustom,fechavalida from otd where orden='$Orden' and estudio='$Estudio'", $link);
...
//	$descripcion= utf8_encode(ucwords(strtolower($Ele[descripcion])));
    ...
    if ($Ele[tipo] == 'n') {
        ...
        if ($Ele[condiciona] == '') {
            ...
            $restexto2 = $Res[$Ele[tipo]]; //respeta salto de linea

            $pdf->SetFont('Helvetica', '', 9, '', 'false');

            $html = '<pre>' . $restexto2 . '</pre>';

            //$html .= '<table width="800" border="0" cellspacing="0" cellpadding="5"><tr style="background-color: ' . $Fdo . ';color: #000;"><td width="20" align="rigth"></td><td width="400" align="center">' . $descripcion . '</td><td width="750" align="left" height="30" colspan="6"><pre>' . $restexto . '</pre></td></tr></table>';
        } else {
