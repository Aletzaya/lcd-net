...
if (isset($_REQUEST["busca"])) {
    ...
    if ($_REQUEST["busca"] == "ini") {
        ...
        $_SESSION["OnToy"] = array('', '', 'pertec.id', 'Asc', $Retornar);   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
if($Palabras > 1){
 ...
 for ($i = 0; $i < $Palabras; $i++) {
        if(!isset($BusInt)){$BusInt=" pertec.nombre like '%$P[$i]%' ";}else{$BusInt=$BusInt." and pertec.nombre like '%$P[$i]%' ";}
 }
 //$Suc='*';

}else{
        $BusInt=" pertec.nombre like '%$busca%' ";  
// $Suc='*';
}
...
if( $busca == ''){
    ...
    $cSql = "SELECT $Qry[campos] FROM perlab WHERE $BusInt $Qry[filtro]";
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <meta charset="UTF-8">
        ...
        <title>Personal Técnico Radiólogo ::..</title>
