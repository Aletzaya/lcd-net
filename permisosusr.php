...
$op = $_REQUEST["op"];
$Msj = $_REQUEST["Msj"];
$Id = 67;             //Numero de query dentro de la base de datos
...
if($Palabras > 1){
 ...
 for ($i = 0; $i < $Palabras; $i++) {
        if(!isset($BusInt)){$BusInt=" authuser.uname like '%$P[$i]%' ";}else{$BusInt=$BusInt." and authuser.uname like '%$P[$i]%' ";}
 }
 //$Suc='*';

}else{
        $BusInt=" authuser.uname like '%$busca%' ";  
// $Suc='*';
}
...
//Tabla contenedor de brighs
    ...
    while ($rg = mysql_fetch_array($res)) {
        ...
        echo "<td class='Seleccionar' align='center'><a class='edit' href='$cLink?busca=$rg[uname]'><i class='fa fa-pencil fa-2x' aria-hidden='true'></i></a></td>";
