...
class NumericalCurrencyConverter implements NumberToLetterConverter {
    ...
    public function convert($number) {
        ...
        trigger_error ("Converting " . $number);
        if ($number>$this->language->getMaxNumber()) {
            ...
            while ($index>1) { // Iterate over defined triads
                ...
                trigger_error ("Indice " . $index . "<br/>");
                $a = intval($b/$index);
                $b = fmod($b, $index);
                trigger_error ("a = " . $a . "<br/>");
                trigger_error ("b = " . $b . "<br/>");
                ...
                trigger_error("Triada " . $formatedTriad . "<br/>");
                $letter .= $formatedTriad . $posfix;
                trigger_error("Letra ".$letter . "<br/>");
                $next = $this->language->forcePrintNextScale($posfix, $index);
            }
        }
        trigger_error ("Parte decimal " . round($number*100)%100);
