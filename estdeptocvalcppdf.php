...
if ($op == 'rs') {  //Registra resultados
    ...
    $ElX2 = mysql_query("SELECT * FROM resul WHERE estudio='$estudio' and orden='$busca' ORDER BY elemento");

    while ($Elex2 = mysql_fetch_array($ElX2)) {

            $Ressulele2 = $Elex2[elemento];

            $tipos2 = mysql_query("SELECT tipo,min,max,valref,vtexto,vlogico FROM $tabla WHERE estudio='$estudio' and id='$Ressulele2' ");
            $tipo2 = mysql_fetch_array($tipos2);
            ...
            if($tipo2[tipo]=='c'){
                ...
                if ($tipo2[valref] == 'Si') {
                    ...
                    $comparatxt2=strcasecmp($Ressul2,$tipo2[vtexto]);
                ...
                if ($Ressul2 < $tipo2[min] OR $Ressul2 > $tipo2[max]) {
    ...
    if($_REQUEST[Guardar] == "Guardar"){
        ...
        $Up = mysql_query("UPDATE otd SET fechavalida='$Fecha $Hora', status='$Sta', usrvalida='$Gusr', alterno='$alterno', fr='$fr2', creapdf='pdf' WHERE orden='$busca' and estudio='$estudio'");
...
/*
           ...
           // echo "<td align='center' colspan='3'><br><input style='color: #000000; background-color: #F1948A' class='letrap' type='submit' name='Validar' value='Validar'></input> &nbsp &nbsp &nbsp <font color='red'><b> <--- Validar para imprimir --- </b></font></td></tr>";
            ...
            if ($Otd[capturo] <> '') {
                ...
                if ($Otd[status] == 'VALIDADA' or $Otd[status] == 'TERMINADA') {

                        echo "<td align='center' colspan='3'><br><input style='color: #000000; background-color: #F1948A' class='letrap' type='submit' name='Validar' value='Validar'></input> &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp <font color='green'><b> <--- Validada --- <i class='fa fa-check-square-o' aria-hidden='true' style='color:#27AE60'></i></b></font> </td></tr>";
                    ...
                    if($Validaciocero==0){
                        ...
                        echo "<td align='center' colspan='3'><br><input style='color: #000000; background-color: #F1948A' class='letrap' type='submit' name='Validar' value='Validar'></input> &nbsp &nbsp &nbsp <font color='red'><b> <--- Validar --- </b></font></td></tr>";
