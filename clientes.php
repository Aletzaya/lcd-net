...
if ($op == 'Si') {                    //Elimina rg
    ...
    $Up = mysql_query("UPDATE qryslcd SET filtro='' WHERE id=$Id");
    $op = '';
}

#Tomo los datos principales campos a editar, tablas y filtros;
$QryA = mysql_query("SELECT campos,froms,edi,tampag,filtro FROM qryslcd WHERE id=$Id");
...
if( $busca == ''){
    ...
    $cSql = "SELECT $Qry[campos], cli.numveces FROM cli LEFT JOIN zns ON cli.zona=zns.zona";   

}elseif( $busca < 'a'){

    $cSql = "SELECT $Qry[campos], cli.numveces FROM cli LEFT JOIN zns ON cli.zona=zns.zona WHERE cli.cliente='$busca'";

}else{

    $cSql = "SELECT $Qry[campos], cli.numveces FROM cli LEFT JOIN zns ON cli.zona=zns.zona WHERE $BusInt";
...
$aDer = array("Visitas","","","", "", "", "", "", "");    //Arreglo donde se meten los encabezados; Derechos;
$tamPag = $Qry[tampag];
...
//Tabla contenedor de brighs
 ...
 //   }
    ...
    while ($rg = mysql_fetch_array($res)) {
        ...
        if ($RetSelec <> '') {
            echo "<td class='Seleccionar' align='center'><a class='edit' href='$RetSelec?Paciente=$rg[cliente]'><i class='fa fa-pencil fa-2x' aria-hidden='true'></i></a></td>";
        } else {
            echo "<td class='Seleccionar' align='center'><a class='edit' href='$cLink?busca=$rg[cliente]'><i class='fa fa-pencil fa-2x' aria-hidden='true'></i></a></td>";
        }
        ...
        echo "<td align='center'><a class='elim' href=javascript:confirmar('No_es_posible_eliminar_clientes_comunicarse_con_soporte_$rg[id]?','$_SERVER[PHP_SELF]?cId=$rg[id]&op=Noo');><i class='fa fa-trash fa-2x' aria-hidden='true'></i> </a></td>";
