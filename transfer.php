...
if (isset($_REQUEST[busca])) {
    ...
    if ($_REQUEST[busca] == ini) {
        ...
        $_SESSION["OnToy"] = array('', '', 'trans.id', 'Asc', $Retornar, $Fecha, $Fecha, '');   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
if($Palabras > 1){
 ...
 for ($i = 0; $i < $Palabras; $i++) {
        if(!isset($BusInt)){$BusInt=" trans.usrdest like '%$P[$i]%' ";}else{$BusInt=$BusInt." and trans.usrdest like '%$P[$i]%' ";}
 }
 //$Suc='*';

}else{
        $BusInt=" trans.usrdest like '%$busca%' ";  
// $Suc='*';
}
...
if( $busca == ''){
    ...
    $cSql = "SELECT $Qry[campos] FROM $Qry[froms] WHERE trans.fecha>='$FechaI' and trans.fecha<='$FechaF'"; 

}elseif( $busca < 'a'){

    $cSql = "SELECT $Qry[campos] FROM $Qry[froms] WHERE  trans.id='$busca'";
...
//Tabla contenedor de brighs
    ...
    while ($rg = mysql_fetch_array($res)) {
        ...
        if ($rg["status"] === "ABIERTA") {
            echo "<td class='Seleccionar' align='center'><a class='edit'><i class='fa fa-print fa-2x' style='color:red' aria-hidden='true'></i></a></td>";
        } else {
            echo "<td class='Seleccionar' align='center'><a class='edit' href=javascript:winmed('imptransferpdf.php?busca=$rg[id]');><i class='fa fa-print fa-2x' aria-hidden='true'></i></a></td>";
        }
        echo "<td class='Seleccionar' align='center'><a class='edit' href='$cLink?busca=$rg[id]'><i class='fa fa-list-alt fa-2x' aria-hidden='true'></i></a></td>";
