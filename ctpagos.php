...
if (isset($_REQUEST["busca"])) {
    ...
    if ($_REQUEST["busca"] === "ini") {
        ...
        $_SESSION["OnToy"] = array('', '', 'dpag_ref.id', 'Asc', $Retornar);   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
$op = $_REQUEST["op"];
$Msj = $_REQUEST["Msj"];
$Id = 56;             //Numero de query dentro de la base de datos
...
if($Palabras > 1){
 ...
 for ($i = 0; $i < $Palabras; $i++) {
        if(!isset($BusInt)){$BusInt=" dpag_ref.concept like '%$P[$i]%' ";}else{$BusInt=$BusInt." and  dpag_ref.concept like '%$P[$i]%' ";}
 }
 //$Suc='*';

}else{
        $BusInt=" dpag_ref.concept like '%$busca%' ";  
// $Suc='*';
}
...
if( $busca == ''){
    ...
    $cSql = "SELECT $Qry[campos],dpag_ref.cerrada FROM $Qry[froms] WHERE id>= 0"; 

}elseif( $busca < 'a'){

    $cSql = "SELECT $Qry[campos],dpag_ref.cerrada FROM $Qry[froms] WHERE  id='$busca'";

}else{
    
    $cSql = "SELECT $Qry[campos],dpag_ref.cerrada FROM $Qry[froms] WHERE $BusInt";
...
if ($_REQUEST["op"] === "Si") {
    $Sql = "DELETE FROM dpag_ref WHERE id = " . $_REQUEST["cId"];
    if (!mysql_query($Sql)) {
            ...
            <tr>
                <td height="380" valign="top">
                    ...
                    while ($rg = mysql_fetch_array($res)) {
                        ...
                        echo "<td align='center'><a class='edit' href='$cLink?busca=$rg[id]'><i class='fa fa-pencil fa-2x' aria-hidden='true'></i></a></td>";
                        if ($rg[cerrada] == '1') {
                            echo "<td align='center'><a class='edit' href=javascript:winmed('impgastos.php?busca=$rg[id]')><i class='fa fa-print fa-2x' aria-hidden='true'></i></a></td>";
                        } else {
                            echo "<td align='center'><a class='edit'> - </a></td>";
                        }
