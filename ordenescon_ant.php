...
if (isset($_REQUEST[busca])) {
    ...
    if ($_REQUEST[busca] == ini) {
        ...
        $_SESSION["OnToy"] = array('', '', 'ot.orden', 'Asc', 'menu.php', $Fecha, $Fecha, '*', '', 2, '');   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
if ($GnSuc == '*') {               //Que sucursal estoy checando
    if ($GTodo == '*') {
        $cSql = "SELECT $Qry[campos],cli.numveces,ot.suc,ot.encaja,ot.status,ot.pagada,ot.entemailpac,
            ot.entemailmed,ot.entemailinst,IF (ot.pagada = 'No','F5B7B1','') statuspago,cli.cliente  
            FROM inst,ot LEFT JOIN cli ON ot.cliente=cli.cliente 
            WHERE ot.institucion=inst.institucion $CorreoEnv $StatusPago AND $BusInt $Qry[filtro]";
    } else {
        $cSql = "SELECT $Qry[campos],cli.numveces,ot.suc,ot.encaja,ot.status,ot.pagada,ot.entemailpac,
            ot.entemailmed,ot.entemailinst,IF (ot.pagada = 'No','F5B7B1','') statuspago,cli.cliente 
            FROM inst,ot LEFT JOIN cli ON ot.cliente=cli.cliente 
            WHERE ot.institucion=inst.institucion $CorreoEnv $StatusPago AND ot.fecha >= '$FechaI' AND ot.fecha <= '$FechaF' "
                . "AND $BusInt $Qry[filtro]";
    }
} elseif ($GTodo == '*') { //todo
    $cSql = "SELECT $Qry[campos],cli.numveces,ot.suc,ot.encaja,ot.status,ot.pagada,ot.entemailpac,
        ot.entemailmed,ot.entemailinst,IF (ot.pagada = 'No','F5B7B1','') statuspago,cli.cliente 
        FROM inst,ot LEFT JOIN cli ON ot.cliente=cli.cliente 
        WHERE ot.suc='$GnSuc' AND ot.institucion=inst.institucion $CorreoEnv $StatusPago AND $BusInt $Qry[filtro]";
} else {
    $cSql = "SELECT $Qry[campos],cli.numveces,ot.suc,ot.encaja,ot.status,ot.pagada,ot.entemailpac
        ,ot.entemailmed,ot.entemailinst,IF (ot.pagada = 'No','F5B7B1','') statuspago,cli.cliente 
        FROM inst,ot LEFT JOIN cli ON ot.cliente=cli.cliente 
        WHERE ot.institucion=inst.institucion AND ot.suc='$GnSuc' $CorreoEnv $StatusPago AND ot.fecha >= '$FechaI' "
            . "AND ot.fecha <= '$FechaF'  AND $BusInt $Qry[filtro]";
}
//echo $cSql;
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body topmargin="1">
        ...
        <form name='frmfiltro' method='get' action="<?= $_SERVER['PHP_SELF'] ?>">
            ...
            <select class='content5' name='FiltroCia' onChange='frmfiltro.submit();'>
                <?php
            ...
            <input type='date' name='FechaI' value='<?= $FechaI ?>' size='10' class='content5'></input> 
            <span class='content1'> Fec.final: </span>
            <input type='date' name='FechaF' value='<?= $FechaF ?>' size='10' class='content5'></input> 
           <!--<span class='content1'>Envio Correo :</span>
            <select class="content5" name="EnvioCorreo">
                <option value="0">No</option>
                <option value="1">Si</option>
                <option value="<?=$EnvioCorreo?>" selected><?=$EnvioCorreoNombre?></option>
            </select>
             <span class='content1'>Pagado :</span>
            <select class="content5" name="Pagado">
