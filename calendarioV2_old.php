...
', ' ') ubicacion,generadora,receptora,fcita,frealizacion,usr,maps"
            . " FROM calendario WHERE id >= 0 ORDER BY id DESC ";
...
', ' ') observaciones,replace(ubicacion, '
', ' ') ubicacion,generadora,receptora,fcita,frealizacion,usr,maps"
            . " FROM calendario WHERE id >= 0 AND sucursal='1' ORDER BY id DESC";
    $req = $bdd->prepare($sql);
    $req->execute();
...
', ' ') observaciones,replace(ubicacion, '
', ' ') ubicacion,generadora,receptora,fcita,frealizacion,usr,maps"
            . " FROM calendario WHERE id >= 0 AND sucursal='2' ORDER BY id DESC";
...
', ' ') observaciones,replace(ubicacion, '
', ' ') ubicacion,generadora,receptora,fcita,frealizacion,usr,maps"
            . " FROM calendario WHERE id >= 0 AND sucursal='3' ORDER BY id DESC";
...
', ' ') observaciones,replace(ubicacion, '
', ' ') ubicacion,generadora,receptora,fcita,frealizacion,usr,maps"
            . " FROM calendario WHERE id >= 0 AND sucursal='4' ORDER BY id DESC";
...
', ' ') observaciones,replace(ubicacion, '
', ' ') ubicacion,generadora,receptora,fcita,frealizacion,usr,maps"
            . " FROM calendario WHERE id >= 0 AND sucursal='5' ORDER BY id DESC";
...
', ' ') observaciones,replace(ubicacion, '
', ' ') ubicacion,generadora,receptora,fcita,frealizacion,usr,maps"
            . " FROM calendario WHERE id >= 0 AND sucursal='6' ORDER BY id DESC";
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    var calendar = new FullCalendar.Calendar(calendarEl, {
                        ...
                        eventDrop: function (info) { // Cambiamos de fecha via JS
                            Swal.fire({
                                title: "¿ Seguro de cambiar " + info.event.title + " al dia " + moment(info.event.start.toISOString()).format("YYYY-MM-DD HH:mm") + " ?",
                                position: "center",
                                icon: "info",
            ...
            <form name='form1' method='get' action="<?= $_SERVER['PHP_SELF'] ?>" onSubmit='return ValidaCampos();'>
                <table width='98%' align='center' border='0' cellpadding='1' cellspacing='2' bgcolor='#f1f1f1' style='border-collapse: collapse; border: 1px solid #999;'>  
                    <tr style = "background-color: #255B83" class = 'letratitulo'>
                        <td align="center" width="30%" class="ssbm">
                            Sucursal:
                                    ...
                                    <td valign="top">
                                        <table border='0' style="width: 95%;">
                                            ...
                                            <tr style="height: 30px;" align="right" class="letrap">
                                                <td  width="100%" class="ssbm">
                                                    <h5 class="letrap" id="Usr">
                                                        <a class="letrap" style="align-content: center;"><div id="Usr"><?= $event[usr] ?></div></a>
                                                    </h5>
                                            </tr>
                                            <!--
                                            <tr style="height: 30px" class="letrap">
