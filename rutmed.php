...
if (isset($_REQUEST["busca"])) {
    ...
    if ($_REQUEST["busca"] == "ini") {
        ...
        $_SESSION["OnToy"] = array('', '', 'ruta.id', 'Asc', $Retornar);   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
    } elseif ($_REQUEST["busca"] == "NUEVO") {  

        $_SESSION["OnToy"] = array('NUEVO', '', 'ruta.id', 'Asc', $Retornar);   //Inicio arreglo(0=busca,1=pagina,2=orden,3=Asc,4=a donde regresa)
...
if($op=='Elim'){                    //Elimina rg
    ...
    $lUp  = "DELETE FROM ruta WHERE id='$_REQUEST[Id]'";
;
    ...
    if (!mysql_query($lUp)) {
        ...
        $sqlD = "INSERT INTO logcat (usr,accion,tabla,fecha,cliente) VALUES ('$Gusr','/Cat Ruta/Elimina Ruta $sqlb[nombre]','Ruta','$Fecha','$_REQUEST[Id]')";
        mysql_query($sqlD);
        header("Location: rutmed.php?busca=ini&Msj=$Msj");
    }
}
...
$sql = "UPDATE ruta set descripcion='$_REQUEST[Descripcion]' WHERE id= $_REQUEST[deptos]";

if (!mysql_query($sql)) {
    $Msj = "Error en sintaxis MYSQL " . $sql;
} else {
    $Msj = "Registro Actualizado con Exito";
    $sqlD = "INSERT INTO logcat (usr,accion,tabla,fecha,cliente) VALUES ('$Gusr','/Cat Ruta/Actualiza Ruta $_REQUEST[Descripcion]','Ruta','$Fecha','$_REQUEST[deptos]')";
    mysql_query($sqlD);
    header("Location: rutmed.php?busca=ini&Msj=$Msj");
}
...
$sql = "INSERT INTO ruta(descripcion) VALUES ('$_REQUEST[Descripcion]') limit 1;";
...
if (mysql_query($sql)) {
    ...
    $sqlD = "INSERT INTO logcat (usr,accion,tabla,fecha,cliente) VALUES ('$Gusr','/Cat Ruta/Agrega nueva Ruta $_REQUEST[Descripcion]','Ruta','$Fecha','$_REQUEST[deptos]')";
    ...
    header("Location: rutmed.php?busca=ini&Msj=$Msj&Error=SI");
}
...
if($Palabras > 1){
 ...
 for ($i = 0; $i < $Palabras; $i++) {
        if(!isset($BusInt)){$BusInt=" ruta.descripcion like '%$P[$i]%' ";}else{$BusInt=$BusInt." and ruta.descripcion like '%$P[$i]%' ";}
 }
 //$Suc='*';
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <meta charset="UTF-8">
            ...
            <title>Catalogo de Rutas Medicas ::..</title>
...
PonEncabezado();
    ...
    echo '<tr>';
        echo "<td align='center'><a class='edit' href=javascript:winuni('logruta.php')><i class='fa fa-file-code-o fa-1x' aria-hidden='true'></i><font size='1'> Log Ruta </font></a></td>";
    echo '</tr>';
echo '</table>';
