...
*/
$Palabras = explode(" ", $busca);
for($i = 1; $i < count($Palabras); $i++) {
  if(!empty($Palabras[$i])) {
    $BusInt .= " and cli.nombrec like '%" . $Palabras[$i] . "%'";
  }
}
...
if ($busca == '') {
    ...
    $cSql = "SELECT ot.institucion,ot.orden,cli.nombrec,ot.fecha,ot.hora,est.estudio,est.descripcion,ot.cliente,ot.entemailmed,ot.entemailinst,otd.usrest,otd.statustom,otd.alterno,otd.estudio,otd.capturo,otd.fr,otd.creapdf,otd.usrvalida,est.depto,otd.proceso,otd.status,otd.recibeencaja,ot.suc,ot.entemailpac,ot.observaciones,est.subdepto,cli.numveces
        ...
        INNER JOIN est on est.estudio=otd.estudio
        INNER JOIN cli on cli.cliente=ot.cliente
        WHERE ot.cliente=cli.cliente $BusInt";
}
...
$aDer = array("Status", "-", "-","ObsG", "-", "-","Correo", "-", "-", "Tom/Real", "-", "-", "Ent.Recep", "-", "-", "Vcs", "-", "-");    //Arreglo donde se meten los encabezados; Derechos;
$tamPag = $Qry[tampag];
                        ...
                        if ($rg[capturo] <> '') {
                            ...
                            if ($rg[depto] <> 2) {
                                ...
                                if ($rg[status] == 'TERMINADA' and $rg[usrvalida] <> '') {
                                    ...
                                    echo "<td class='Seleccionar' align='right'><a class='edit' href=javascript:wingral('resultapdf.php?cInk=$clnk&Orden=$rg[orden]&Estudio=$rg[estudio]&Depto=TERMINADA&op=im&alterno=$rg[alterno]')><i class='fa fa-file-pdf-o fa-lg' aria-hidden='true' style='color:#FF0000'></i></a></td>";
